...
public class FileUtils {
	/**
	 * 获取指定目录下的所有的文件， 返回值是这个文件在指定目录下的相对路径及其文件名
	 * 
	 * @param 查询路径
	 * @param filter
	 ...
	 * 
	 */
	public static List<String> getClassesWholeName(String path,
			FilenameFilter filter, AfterOperate<String, String> afterOperate)
			throws FileNotFoundException {
		...
		if (afterOperate == null) {
			for (Pair<String, File> pair : files) {
				re.add(pair.getFirst() + File.separator
						+ pair.getSecond().getName());
			}
		} else {
			for (Pair<String, File> pair : files) {
				re.add(afterOperate.operate(pair.getFirst() + File.separator
						+ pair.getSecond().getName()));
			}
		}
	...
	private static List<Pair<String, File>> addClassNames(
			Pair<String, File> pair, List<Pair<String, File>> files,
		...
		for (File file : lists) {
			if (file.isDirectory()) {
				reDirs.add(new Pair<String, File>(sTmp + File.separator
						+ dir.getName(), file));
			} else {
				files.add(new Pair<String, File>(sTmp + File.separator
						+ dir.getName(), file));
			}
		}
